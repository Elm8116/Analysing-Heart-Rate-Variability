...
grp = data['id'].values
model =  DecisionTreeClassifier(max_depth=4, criterion='entropy', min_samples_leaf=6,
...
print ("Decision Tree Mean score: {0:.3f} (+/-{1:.3f})").format(np.mean(scores), np.std(scores))


scores_cross = cross_val_score(model, X, Y, cv=10, scoring='accuracy')
print('Decision Tree Cross Validation Score: {0:.3f} (+/-{1:.3f})').format(np.mean(scores_cross), np.std(scores_cross))
...
def conf_mat_logo_aggregated():
    ...
    plot_confusion_matrix(cnf_matrix, classes=['class 0', 'class 1'],
                          title='Decision Tree matrix, without normalization')
