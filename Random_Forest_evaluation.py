...
grp = data['id'].values
model = RandomForestClassifier(n_estimators=148, criterion='entropy', max_depth=12,
...
print ("Random Forest Mean score: {0:.3f} (+/-{1:.3f})").format(np.mean(scores), np.std(scores))


scores_cross = cross_val_score(model, X, Y, cv=10, scoring='accuracy')
print('Random Forest Cross Validation Score: {0:.3f} (+/-{1:.3f})').format(np.mean(scores_cross), np.std(scores_cross))
